...
function convertText(content: string, words: WordItem[]): string {
  ...
  for (let i = 0; i < rubyMatches.length; i++) {
    ...
    for (let j = i; j < rubyMatches.length; j++) {
      // 检查是否连续（中间没有其他字符）
      if (j > i) {
        const prevEnd = rubyMatches[j-1].index + rubyMatches[j-1].fullMatch.length;
  ...
  // 按长度排序，优先匹配长的单词
  wordGroups.sort((a, b) => (b.endIndex - b.startIndex) - (a.endIndex - a.startIndex));
  ...
  while (currentIndex < content.length) {
    // 检查当前位置是否在已处理的单词组中
    const matchingGroup = processedGroups.find(group =>
    ...
    if (matchingGroup) {
      ...
      // 检查当前位置是否有未处理的注音
      const rubyMatch = rubyMatches.find(m => m.index === currentIndex && !usedIndices.has(m.index));
